...
# 定义数据库类型（可配置）
DB_TYPE = "SQLLite"  # 可选值: "MySQL", "PostgreSQL", "SQLite", "Oracle", "SQL Server"
...
dbAccess = DBAccess(dialect=DB_TYPE)
...
# 添加新的辅助函数，用于刷新所有缓冲消息
async def flush_message_buffers(collector: StreamResponseCollector) -> None:
    """确保所有缓冲区中的消息都被发送出去
    
    Args:
        collector: 消息收集器实例
    """
    if not collector.callback:
        return
        
    # 遍历所有缓冲区，发送剩余内容
    for source, content in collector.message_buffers.items():
        if content.strip():  # 只发送非空内容
            message = ResponseMessage(
                source=source,
                content=content
            )
            await collector.callback(message)
    
    # 清空所有缓冲区
    collector.message_buffers.clear()
    collector.last_flush_time.clear()
