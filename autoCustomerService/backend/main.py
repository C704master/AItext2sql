...
from config import settings  # 修改为导入 settings 实例
import os

# 验证配置是否正确加载
if not hasattr(settings, "MINIO_ENDPOINT"):
    raise AttributeError("配置缺少 MINIO_ENDPOINT 属性，请检查 config.py 文件。")
if not hasattr(settings, "MINIO_ACCESS_KEY"):
    raise AttributeError("配置缺少 MINIO_ACCESS_KEY 属性，请检查 config.py 文件。")
if not hasattr(settings, "MINIO_SECRET_KEY"):
    raise AttributeError("配置缺少 MINIO_SECRET_KEY 属性，请检查 config.py 文件。")
if not hasattr(settings, "MINIO_BUCKET"):
    raise AttributeError("配置缺少 MINIO_BUCKET 属性，请检查 config.py 文件。")
