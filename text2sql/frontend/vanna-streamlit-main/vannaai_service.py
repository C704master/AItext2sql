...
from vanna.exceptions import ImproperlyConfigured, DependencyError, ValidationError
...
class MyVanna(ChromaDB_VectorStore, OpenAI_Chat):
    ...
    def generate_sql_v2(self,
        ...
        # 得到大模型的SQL生成回答
        llm_response = self.submit_prompt(prompt=prompts, model=openai_model)

        # 提取大模型的SQL生成回答中的SQL语句
        return self.extract_sql(llm_response)

    #
    def _extract_python_code_v2(self, markdown_string: str) -> str:
        # 去掉空格以避免llm生成的代码中的缩进错误
    ...
    def generate_plotly_code_v2(
            self, question: str = None, sql: str = None, df_metadata: str = None, visual_command:str =None,**kwargs
    ) -> str:
        if question is not None:
            system_msg = f"下面是一个pandas DataFrame，它包含了回答用户问题的查询结果： '{question}'"
        else:
            system_msg = "下面是一个pandas DataFrame "

        if sql is not None:
            system_msg += f"\n\nDataFrame是使用这个查询生成的: {sql}\n\n"

        system_msg += f"以下是关于生成的pandas DataFrame ‘df’的信息: \n{df_metadata}"

        # 根据visual_command生成plotly画图代码

        if visual_command is not None:
            system_msg += f"\n\n这个是用户对生成的pandas DataFrame的可视化配置推荐: {visual_command}\n\n"

        message_log = [
            self.system_message(system_msg),
            self.user_message(
                "你可以生成Python绘图代码来绘制数据框架的结果吗？假设数据位于名为“df”的pandas数据框中。如果数据框中只有一个值，则使用指示器。只使用Python代码响应。不要回答任何解释——只回答代码。"
            ),
        ]
    ...
    #   连接数据库，同时执行SQL，返回MySQL数据结果
    def connect_to_mysql_v2(
            self,
            host: str = None,
            dbname: str = None,
            user: str = None,
        ...
        try:
            ...
            raise DependencyError(
                "需安装PyMySQL依赖，运行命令：pip install PyMySQL"
            )
        ...
        if not host:
            raise ImproperlyConfigured("请设置MySQL主机")

        if not dbname:
            dbname = os.getenv("DATABASE")

        if not dbname:
            raise ImproperlyConfigured("请设置MySQL数据库")

        if not user:
            user = os.getenv("USER")

        if not user:
            raise ImproperlyConfigured("请设置MySQL用户")

        if not password:
            password = os.getenv("PASSWORD")

        if not password:
            raise ImproperlyConfigured("必须设置MySQL密码")

        if not port:
            port = os.getenv("PORT")

        if not port:
            raise ImproperlyConfigured("请设置MySQL端口")
        ...
        # 这里是一个函数指针，到时候调用run_sql()函数，就可以执行这个函数了
        ...
        self.run_sql_result = run_sql_mysql
...
def main():
    ...
    test_result = vn.generate_sql_question_report("Please give me the information of all the albums")
    print(test_result)
